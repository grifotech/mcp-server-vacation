...
export const tokenData = { secret: null, payload: null };
...
export class MCPServer {
    ...
    async streamMessages(transport) {
        try {
            const message = { method: "notifications/message", params: { level: "info", data: "SSE Connection established" } };
            this.sendNotification(transport, message);
            let messageCount = 0;
            const interval = setInterval(() => {
                messageCount++;
                const data = `Message ${messageCount} at ${new Date().toISOString()}`;
                const msg = { method: "notifications/message", params: { level: "info", data } };
                this.sendNotification(transport, msg);
                if (messageCount === 2) {
                    clearInterval(interval);
                    const done = { method: "notifications/message", params: { level: "info", data: "Streaming complete!" } };
    ...
    async sendNotification(transport, notification) {
        const rpcNotificaiton = { ...notification, jsonrpc: "2.0" };
