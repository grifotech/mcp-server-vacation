...
import { validateJWT, extractTokenInfo } from "./jwt-utils.js";
...
router.post(MCP_ENDPOINT, authMiddleware, async (req: Request, res: Response) => { await server.handlePostRequest(req, res); });
router.get(MCP_ENDPOINT, authMiddleware, async (req: Request, res: Response) => { await server.handleGetRequest(req, res); });

// Endpoint de teste sem autenticação
router.post(MCP_TEST_ENDPOINT, async (req: Request, res: Response) => { await server.handlePostRequest(req, res); });
router.get(MCP_TEST_ENDPOINT, async (req: Request, res: Response) => { await server.handleGetRequest(req, res); });
